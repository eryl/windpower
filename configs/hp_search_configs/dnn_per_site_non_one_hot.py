...
from windpower.dnn_models import NNTabularModel, NNTabularConfig, TorchWrapper, TorchWrapperConfig

import windpower.mltrain.train
from windpower.mltrain.hyperparameter import DiscreteHyperParameter, LinearHyperParameter
...
torch_model = NNTabularModel

model_args = (nnconfig,)
model_kwargs = dict()

wrapper_config = TorchWrapperConfig(batch_size=batch_size,
                                    device=torch.device('cpu'),
                                    model_class=torch_model,
                                    model_args=model_args,
                                    model_kwargs=model_kwargs)
model = TorchWrapper
wrapper_args = (wrapper_config,)
wrapper_kwargs = dict()
...
train_kwargs = windpower.mltrain.train.TrainingConfig(max_epochs=300, keep_snapshots=False)
