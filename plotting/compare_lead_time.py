...
def main():
    ...
    for model in sorted(models):
        ...
        plt.subplots_adjust(top=0.92,
                            ...
                            hspace=0.2,
                            wspace=0.2)
        plt.suptitle(f'{model}, With lead time as a feature')
        joint.ax_joint.set_xlim(x_min, x_max)
        joint.ax_joint.set_ylim(y_min, y_max)
        if args.output_dir is not None:
            save_path = args.output_dir / f'lead_time_{model}.png'
        ...
        plt.subplots_adjust(top=0.92,
                            ...
                            hspace=0.2,
                            wspace=0.2)
        plt.suptitle(f'{model}, Without lead time as feature')
        joint.ax_joint.set_xlim(x_min, x_max)
        joint.ax_joint.set_ylim(y_min, y_max)
        if args.output_dir is not None:
            save_path = args.output_dir / f'lead_time_{model}.png'
