...
def main():
    ...
    fig, axes = plt.subplots(len(nwp_models), 1, sharex='col', squeeze=False)
    for i, nwp_model in enumerate(sorted(nwp_models)):
        for j, model in enumerate(sorted(models)):
            ...
            sns.distplot(model_df['mean_absolute_error'], label=model, ax=ax,
                         bins=50, hist_kws=dict(range=(mae_min, mae_max)), rug=False)
