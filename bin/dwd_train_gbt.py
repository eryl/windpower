...
def main():
    ...
    parser.add_argument('--learning-rate-values', help="Values to consider for learning rate", nargs='+',
                        type=float, default=[1e-3, 1e-2, 1e-1, 1])
    parser.add_argument('--subsample-values', help="Values to consider for the subsampling", nargs='+',
                        type=float, default=[0.25, 0.5, 0.75, 1])
    parser.add_argument('--max-features-values', help="Values to consider for max features", nargs='+',
                        type=float, default=[0.25, 0.5, 0.75, 1])
    ...
    for site_dataset_path in tqdm(args.site_files):
        ...
        learning_rate = DiscreteHyperParameter(args.learning_rate_values)
        subsample = DiscreteHyperParameter(args.subsample_values)
        max_features = DiscreteHyperParameter(args.max_features_values)
