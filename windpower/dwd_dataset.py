...
def make_variable_definitions(variables, one_hot_encode=False):
    default_map = {'T': Variable('T'),
                   'U': Variable('U'),
                   'V': Variable('V'),
                   'phi': DiscretizedVariableEvenBins('phi', (-np.pi, np.pi), 64, one_hot_encode=one_hot_encode),
                   'r': Variable('r'),
                   'site_production': Variable('site_production'),
                   'time_of_day': CategoricalVariable('time_of_day', levels=np.arange(24),
                                                      mapping={i:i for i in range(24)}, one_hot_encode=one_hot_encode),
...
class SiteDataset(object):
    def __init__(self, *, dataset_path: Path,  window_length, production_offset, dataset=None, horizon=None,
        ...
        self.variable_definitions = {v:make_variable_definitions(v, one_hot_encode=one_hot_encode)
                                     for v in weather_variables}
        self.use_cache = use_cache
        if self.include_lead_time:
            self.variable_definitions['lead_time'] = make_variable_definitions('lead_time',
                                                                               one_hot_encode=one_hot_encode)
        if self.include_time_of_day:
            self.variable_definitions['time_of_day'] = make_variable_definitions('time_of_day',
                                                                                 one_hot_encode=one_hot_encode)
        self.variable_definitions[production_variable] = make_variable_definitions(production_variable)
        self.setup_xref()
        if self.use_cache:
            self.setup_cache()
    ...
    def k_fold_split(self, k, padding=12):
        ...
        for i in range(len(fold_intervals)):
            ...
            fold = SiteDataset(dataset=self.dataset, reference_time=fold_times, **kwargs)
            remainder = SiteDataset(dataset=self.dataset, reference_time=remainder_times, **kwargs)
